...
from test_cases.Models.Doexcel import ReadExcel
from test_cases.Models.logtest import Logger

log = Logger(__name__, CmdLevel=logging.INFO, FileLevel=logging.INFO)

eleData = ReadExcel()  # 存储系统所有的元素数据
testLoginData = ReadExcel('elementDate.xlsx', 'elementsInfo')  # 登录模块测试数据


class BasePage:
    def __init__(self, driver):
        log.logger.info('浏览器正常启动了')
        self.driver = driver

    def open(self, url):
        log.logger.info('正常启动了{}'.format(url))
    ...
    def input_(self, loc, txt):
        log.logger.info('打开了{}元素，输入{}字符'.format(loc, txt))
